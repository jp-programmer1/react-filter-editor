...
const SelectCustom = ({ data, onChange }) => {
    var _a;
    return (react_1.default.createElement("input", { type: "select", name: data.name, id: data.name, className: "form-control", value: (_a = data.value) === null || _a === void 0 ? void 0 : _a.toString(), onChange: (e) => onChange(e.target.value) }, data.options && data.options.map(op => (react_1.default.createElement("option", { value: op.value.toString() }, op.label)))));
};
const InputCustom = ({ data, onChange }) => {
    var _a;
    return (react_1.default.createElement("input", { autoComplete: 'off', className: "form-control", 
        //@ts-ignore
